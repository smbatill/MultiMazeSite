...
updated for website
 ...
 - added display Solution option
 - added a solved function
...
//let string s1,s2,s3,s4,s5,s6,s7,s8;
...
function buildMaze () {
  ...
  iStepUp=false; iStepDwn=false; iStepRgt=false; iStepLft=false;
    ...
    nRows = nColsMin + int(((sizeBar.getPos()-913)/150)*65);
    nCols = nRows;

    size = .9*height/nRows;
    let dum5=sizeBar.getPos();
  ...
  for (let i = 0; i < nRows; i++) {
    ...
    for (let j = 0; j < nCols; j++) {
      cell[i][j] = new Cell(originX+(j * size), (originY+size/2)+(i * size), size, true, true, true, true,false,true, true, true, true,false,false);
    }
  }
// markers for solution 
  for (let i = 0; i < nRows; i++) {
      pathCellIndx[i]=[];
    for (let j = 0; j < nCols; j++) {
     pathCellIndx[i][j] = new Path(originX+(j * size), (originY+size/2)+(i * size),false,dotSize);
    }
  }
    ...
    let xInDoor = cell[0][inDoor].xCntr;
    let yInDoor = cell[0][inDoor].yCntr - (size/2) - 25;

    let xOutDoor = cell[nRows-2][outDoor].xCntr;
    let yOutDoor = cell[nRows-2][outDoor].yCntr + (size/2) + 5;
...
function solveMaze(){
    ...
    iStpDwn=false; iStpUp=false; 
    iStpRgt=false; iStpLft=false;
...
//--------------------------------------------------------------
function drawPath(){
    // draw your path through the maze
...
function findSolution(){
    ...
    cell[0][jSol].swall3 = true; // close the top boarder  entry cell wall3
    cell[iSol][jSolStart].iSealed = false; // and the starting cell can't be sealed   
    cell[0][jSol].iSealed = true; // seal the top boarder entry cell
    solStepCnt = 0; 
    iStopSolution = false;
...
while(iStopSolution == false) {
    ...
    // console.log('start i- ',iSol,' j- ',jSol);
    iStepped = false;
...
*/
 ...
 //   console.log('u-',uCstat,'d-',dCstat,'l-',lCstat,'r-',rCstat);
...
// determine how many open (inPath) cells surround you
    ...
    if((nPathC => 1) && (iStepped == false)){ // find a "random" open (in-solutiom) cell to move into
        ...
        cell[1][jSolStart].inSol = true;  // starting cell always in the solution path
        cell[1][jSolStart].isSealed = false; // enty cell can't be sealed or modified
        cell[1][jSolStart].swall1 = true; // entry top wall is always there - no easy escape

        if((iSol == nRows-2 && jSol == outDoor) || (solStepCnt > 25000)) {iStopSolution = true;}
    ...
    for(let i=0; i<nRows-1; i++){ //each row
        for (let j=0; j<nCols-1; j++) { // across the columns
           if (cell[i][j].inSol == true){
              fill(0,0,255);
              noStroke();
              ellipse(cell[i][j].xCntr,cell[i][j].yCntr,.5*size,.5*size);
           }                      
        }   
    }
    fill(255,0,0);
    noStroke();
    ellipse(cell[iSol][jSol].xCntr,cell[iSol][jSol].yCntr,.6*size,.6*size);
}
...
function mouseReleased() {   
    ...
    if(uPButton.buttonCheck() == true && (iYou != 0)){iStepUp = true;} 
    if(dNButton.buttonCheck() == true){iStepDwn = true;} 
    if(leftButton.buttonCheck() == true){iStepLft = true;} 
    if(rightButton.buttonCheck() == true){iStepRgt = true;} 
...
class Cell {
  constructor(xCntr, yCntr, s, wall1, wall2, wall3, wall4, inM,swall1,swall2,swall3,swall4,isSealed,inSol) {
    this.xCntr = xCntr;
    this.yCntr = yCntr;
    this.s = s;
    this.wall1 = wall1;
  ...
  displayCell() {
    ...
    if(this.wall1 == true) // wall 1
      {line(this.xCntr-this.s/2, this.yCntr-this.s/2,this.xCntr+this.s/2, this.yCntr-this.s/2)}
    if(this.wall2 == true ) // wall 2
      {line(this.xCntr+this.s/2, this.yCntr-this.s/2,this.xCntr+this.s/2, this.yCntr+this.s/2)}
    if(this.wall3 == true) // wall 3
      {line(this.xCntr+this.s/2, this.yCntr+this.s/2,this.xCntr-this.s/2, this.yCntr+this.s/2)}
    if(this.wall4 == true) // wall 4
      {line(this.xCntr-this.s/2, this.yCntr+this.s/2,this.xCntr-this.s/2, this.yCntr-this.s/2)}
  }
  ...
  drawBoarder() {
    noStroke();
    fill(0,0,0);
    rect(this.xCntr-this.s/2,this.yCntr-this.s/2,this.s,this.s)
  }
...
//    this.spos = this.xpos + this.swidth/2 - this.sheight/2;  // use this line  to initially center the slider in the slider bar
 ...
 //   console.log('this.spos', this.spos);
 //   console.log('this.rato', this.ratio);
    let dum6 = this.spos*this.ratio;
 //   console.log('dum6', dum6);
